...
class Concept_Links():
    ...
    def add_concepts(cls):
        for label, opt in cls.labels.items():
            ...
            so.add_mapped_attribute(cls, f'{label}_concept_id', so.mapped_column(sa.Integer, sa.ForeignKey('concept.concept_id', name=f'fk_{cls.__name__[:5]}_{label[:10]}'), nullable=not opt))
