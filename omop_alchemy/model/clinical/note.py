...
class Note(Base):
    ...
    # strings
    note_title: so.Mapped[Optional[str]] = so.mapped_column(sa.String(250))
    note_text: so.Mapped[Optional[str]] = so.mapped_column(sa.Text)
    note_source_value: so.Mapped[Optional[str]] = so.mapped_column(sa.String(50))
    # numeric
