...
class Visit_Occurrence(Base):
    ...
    discharged_to_concept_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('concept.concept_id', name='vo_fk_7'), nullable=True)
    ...
    discharge_to: so.Mapped[Optional['Concept']] = so.relationship(foreign_keys=[discharged_to_concept_id])
